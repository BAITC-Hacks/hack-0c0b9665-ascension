...
export function mountBudgetStory({ target = window, ownerDocument = document, fetcher = fetch,
  render = renderBudgetStory } = {}) {
  let datasetPromise;
  let calculation = 0;
  let hasScenarioData = true;
  let active = true;
  let activePanel;
  ...
  const onCityChange = event => {
    hasScenarioData = event.detail?.id === 'astana' && event.detail?.hasScenarioData === true;
    if (!hasScenarioData) invalidate();
  };
  const getDataset = () => {
    if (!datasetPromise) datasetPromise = fetcher('/api/dataset', { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(15000) })
      .then(async response => {
  ...
  const onCalculated = event => {
    const container = ownerDocument.getElementById('result-content');
    if (!active || !hasScenarioData || !container || container.closest('[hidden]') || !event.detail?.result?.valid) return;
    const detail = structuredClone(event.detail);
    const ticket = ++calculation;
    container.querySelector('.budget-story')?.remove();
    const panel = create('section', 'budget-story');
    activePanel = panel;
    container.append(panel);
    const current = () => active && hasScenarioData && ticket === calculation && panel.isConnected
      && container.contains(panel) && !container.closest('[hidden]');
    const load = async () => {
      if (!current()) return;
      panel.setAttribute('aria-busy', 'true');
      panel.replaceChildren(create('p', '', 'Готовим объяснение бюджета…'));
      try {
        const dataset = await getDataset();
        if (!current()) return;
        render(panel, buildBudgetStory(dataset, detail.scenario, detail.result));
      } catch {
        if (!current()) return;
        panel.removeAttribute('aria-busy');
        const message = create('p', '', 'Объяснение бюджета пока недоступно. Результат расчёта выше сохранён.');
        message.setAttribute('role', 'status');
        const retry = create('button', 'bs-retry', 'Повторить объяснение бюджета');
        retry.type = 'button';
  ...
  const dispose = () => {
    active = false;
    invalidate();
    target.removeEventListener('scenario:calculated', onCalculated);
    target.removeEventListener('scenario:invalidated', invalidate);
    target.removeEventListener('city:changed', onCityChange);
    target.removeEventListener('pagehide', onPageHide);
  };
  const onPageHide = event => { if (!event.persisted) dispose(); };
  target.addEventListener('scenario:calculated', onCalculated);
  target.addEventListener('scenario:invalidated', invalidate);
  target.addEventListener('city:changed', onCityChange);
