...
const sectionIds = [...document.querySelectorAll('.sidebar .nav-item[href^="#"]')].map(link => link.hash.slice(1));
...
function followHash(scroll = false) {
  ...
  if (!sectionIds.includes(id)) { activate('map-section'); return; }
  activate(id);
...
window.addEventListener('scroll', () => {
  ...
  requestAnimationFrame(() => {
    ...
    const sections = sectionIds.map(id => document.getElementById(id)).filter(section => section && section.getClientRects().length).sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top);
