...
async function action(button, fn, modal = false) {
  ...
  button.disabled = true; const original = button.textContent; button.textContent = 'Подождите…';
  ...
  finally { button.disabled = button.id === 'prev' ? page === 1 : button.id === 'next' ? page * 10 >= total : false; button.textContent = original; }
}
...
async function refresh() {
  ...
  total = data.total; if(page > 1 && (page - 1) * 10 >= total) { page = Math.max(1,Math.ceil(total/10)); return refresh(); }
  ...
  $('list').innerHTML = data.items.length ? data.items.map(i => `<article class="row"><span class="muted">№ ${i.id}<br>Демо</span><div class="row-copy"><p class="summary">${esc(i.text.slice(0,150))}${i.text.length>150?'…':''}</p><p class="muted">${esc(district(i.districtId))} · ${esc(config.categories[i.category])}</p><p class="muted">${date(i.createdAt)} · ${esc(i.assignee || 'Не назначен')}</p></div><span class="badge">${esc(config.statuses[i.status])}</span><span class="muted">${i.priority==='high'?'Высокий приоритет':'Обычный приоритет'}</span><button data-open="${i.id}" aria-label="Открыть обращение ${i.id}">Открыть</button></article>`).join('') : '<div class="empty"><h3>Обращений не найдено</h3><p>Измените фильтры или создайте демонстрационное обращение.</p></div>';
...
$('scenarios').onclick=e=>{
  ...
  if(selected.length<2){$('saved-comparison').textContent='Выберите ещё один сценарий для сравнения.';return;}
