...
import { mountScenarioLibrary } from './dashboard-scenario-library.js';
...
function renderPlan() {
  ...
  $('undo-plan').disabled = state.busy || state.demoRunning || !state.history?.length;
  $('calculate-example').disabled = state.busy || state.simulating || state.demoRunning;
}
...
function resetScenario() {
  ...
  if (state.decisions.length) state.history = [...state.history, scenario()].slice(-20);
  state.decisions = [];
  state.totalCost = 0;
  state.filter = 'all';
...
$('calculate-example').addEventListener('click', () => void runDemo());
...
async function initialize() {
  try {
    ...
    const sharedWorkspace = mountTeamWorkspace($('team-workspace-panel'), {
      getDocument: () => actions.getDocument(),
      applyDocument: (document) => actions.applyDocument(document),
    });
    workspaceDisposers.push(() => sharedWorkspace.dispose());
