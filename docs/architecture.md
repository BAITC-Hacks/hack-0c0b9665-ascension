...
Приложение — модульный монолит с двумя HTTP-адаптерами: Node.js для локального запуска и Cloudflare Worker для публичного демо. Оба используют один контракт API симулятора и одно детерминированное ядро. Node также подключает отдельный модуль обращений граждан с файловым хранилищем и Telegram; этот модуль не подключён к Worker. Основной критерий разделения — возможность изменить транспорт или способ объяснения, сохранив формулу, валидацию и HTTP-поведение симулятора.
...
| `src/complaints/` | Отдельные Node-маршруты обращений, хранилище, Telegram и доступ администратора | Подключаются в composition root до общего API; не импортируются в Worker и не меняют расчётное ядро |
