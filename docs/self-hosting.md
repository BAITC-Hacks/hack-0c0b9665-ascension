...
Для локального polling оставьте `PUBLIC_BASE_URL` пустым или укажите адрес именно локальной инсталляции. Облачное DO-хранилище и локальный JSON независимы: ссылка на облачный сайт не сможет найти локальную квитанцию. `http://127.0.0.1` открывается только на компьютере с сервером, не на телефоне жителя. Для облачного варианта с постоянными черновиками см. [Telegram: хостинг и локальный запуск](telegram-hosting.md). В стандартном Node-сервере черновики остаются в памяти; долговременное хранилище сессий ему не подключено.

Если владелец переносит один бот между облаком и своим компьютером: остановить прежний polling, запустить локальный сервер, затем выполнить `npm run bot:local`. Команда проверяет локальный сервер и секрет, снимает webhook и запускает единственный polling. Для возвращения в облако остановить локальный polling (Ctrl+C), задать облачный `TELEGRAM_HOSTED_URL` и выполнить `npm run bot:hosted`. Эта переменная задаёт цель webhook отдельно от `PUBLIC_BASE_URL`, который используется сервером в квитанциях; если `TELEGRAM_HOSTED_URL` пуст, используется `PUBLIC_BASE_URL`. Не запускайте оба режима параллельно; уже принятые обращения остаются в хранилище той инсталляции, где они были созданы. Команды переключения не переносят обращения или черновики между хранилищами.
...
`BOT_SERVER_URL` должен указывать на уже работающий сервер: по умолчанию `http://127.0.0.1:3000`; исправьте при другом `PORT`. Оба процесса должны оставаться включёнными. Polling читает Telegram и передаёт события HTTP-серверу; только веб-сервер записывает JSON. `npm run bot` запускает только polling и отказывается работать при активном webhook, не переключая режим автоматически.

Для systemd сначала при работающей веб-службе выполните `sudo node --env-file=/etc/ascension.env /opt/ascension/src/telegram-mode.js local`. Дождитесь сообщения о запуске polling, остановите этот ручной процесс Ctrl+C, затем включите постоянную службу `sudo systemctl enable --now ascension-bot`. Проверка режима: `sudo node --env-file=/etc/ascension.env /opt/ascension/src/telegram-mode.js status`.

Для Docker при уже работающем `app` сначала явно переведите своего бота на эту инсталляцию:

```sh
docker compose --env-file .env.local exec app node src/telegram-mode.js local
```

Дождитесь запуска polling и остановите ручной процесс Ctrl+C. Затем включите постоянный контейнер:

```sh
docker compose --env-file .env.local --profile telegram up -d --build
```

Контейнер `bot` делит сеть с `app` и обращается к нему через loopback. Он не монтирует volume обращений. Проверка без Node на хосте: `docker compose --env-file .env.local exec app node src/telegram-mode.js status`. В шаблонах systemd и Compose автоматический рестарт запускает только polling; включённый webhook самопроизвольно не снимается.
...
Для переданной резервной копии есть отдельная **локальная проверка без импорта**:

```sh
node scripts/check-complaint-backup.js "/path/to/telegram-complaints.json"
```

Она проверяет формат и целостность по тому же валидатору, который использует сервер. Выводит только версию схемы, количество записей по источникам и SHA-256 файла; текст, адреса, Telegram ID и личные коды не выводятся. Файл не изменяется и никуда не отправляется. Успех этой проверки не означает, что данные перенесены в облако: перенос старых обращений требует отдельного контролируемого импорта и проверки исходной квитанции. Не публикуйте сам JSON и не помещайте его в Git или `public/`.
