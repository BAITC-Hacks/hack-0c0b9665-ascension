...
import { createComplaintStore, toAdminComplaint, toPublicComplaint } from './store.js';
...
export function createComplaintRoutes(options = {}) {
  const store = options.store ?? createComplaintStore({ filePath: process.env.COMPLAINTS_FILE || DEFAULT_FILE });
  ...
  const username = options.telegramUsername ?? process.env.TELEGRAM_BOT_USERNAME ?? '';
