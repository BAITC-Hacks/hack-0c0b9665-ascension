...
const CREATE_FIELDS = ['text', 'address', 'districtId', 'location', 'consent', 'source',
  'telegramChatId', 'telegramUpdateId', 'attachments'];
...
function normalizeInput(input) {
  ...
  if (source === 'web' && ['telegramChatId', 'telegramUpdateId', 'attachments'].some(key => key in input)) {
  ...
  try {
    normalizeInput({ text: record.text, address: record.address, districtId: record.districtId,
      ...
      ...(record.source === 'telegram' ? { telegramChatId: record.telegramChatId,
        telegramUpdateId: record.telegramUpdateId, attachments: record.attachments } : {}) });
...
/** Validate every record before exposing or mutating durable state. */
export function validateComplaintState(state) {
  try {
    if (!plain(state) || state.version !== 1 || !Array.isArray(state.complaints)
      || !state.complaints.every(validPersisted)) throw new Error('Invalid state');
    const ids = new Set();
    const updates = new Set();
    for (const record of state.complaints) {
      if (ids.has(record.id) || (record.source === 'telegram' && updates.has(record.telegramUpdateId))) throw new Error('Duplicate key');
      ids.add(record.id);
...
/**
 * Shared domain methods. The adapter runs each callback with freshly loaded records
 * and an awaited save(records) inside one serialized/transactional boundary.
 */
export function createComplaintStoreCore({ run }) {
  return {
    create(input) {
      return run(async (records, save) => {
    ...
    getTelegramReceipt(incomingId, chatId) {
      return run(records => {
        const record = records.find(item => item.source === 'telegram' && item.telegramUpdateId === incomingId
          && item.telegramChatId === String(chatId));
        return record ? { complaint: structuredClone(record), trackingToken: record.trackingToken, duplicateUpdate: true } : null;
      });
    },
    track(id, token) {
      return run(records => {
